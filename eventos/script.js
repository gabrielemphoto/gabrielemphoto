document.addEventListener("DOMContentLoaded", function() {
  ...
  var viejoLightbox = document.getElementById('lightbox');
  if(viejoLightbox) { viejoLightbox.parentNode.removeChild(viejoLightbox); }

  // CREAR EL NUEVO LIGHTBOX DINÁMICAMENTE
  var modalLightbox = document.createElement('div');
  modalLightbox.className = 'lightbox-modal';
  ...
  var imgModal      = modalLightbox.querySelector('img');
  var cerrarModalBtn = modalLightbox.querySelector('.cerrar-modal');
  var flechaIzq     = modalLightbox.querySelector('.flecha-izq');
  var flechaDer     = modalLightbox.querySelector('.flecha-der');
  var contadorModal = modalLightbox.querySelector('.contador-modal');
  var cargando      = modalLightbox.querySelector('.cargando');
  ...
  function abrirLightbox(index) {
    ...
    setTimeout(function() { modalLightbox.style.opacity = '1'; }, 10);
    imgModal.src = imagenesActuales[indiceActual];
    imgModal.onload = function() { cargando.style.display = 'none'; imgModal.style.opacity = '1'; };
  ...
  function cerrarLightbox() {
    ...
    setTimeout(function() { modalLightbox.style.display = 'none'; }, 300);
    document.body.style.overflow = ''; 
  }

  // ABRIR GALERÍA AL HACER CLIC
  var triggers = document.querySelectorAll('.galeria-trigger');
  for (var j = 0; j < triggers.length; j++) {
    triggers[j].addEventListener('click', function() {
      var cat = this.getAttribute('data-categoria');
      var fotoSola = this.getAttribute('data-src') || this.querySelector('img').src;
      imagenesActuales = galeriasPorCategoria[cat] ? galeriasPorCategoria[cat] : [fotoSola];
      abrirLightbox(0);
    });
  }
  ...
  modalLightbox.addEventListener('click', function(e) { if (e.target === modalLightbox) cerrarLightbox(); });
  ...
  // 3. ACORDEÓN
  var accordionBtns = document.querySelectorAll('.accordion-header');
  for (var k = 0; k < accordionBtns.length; k++) {
    accordionBtns[k].addEventListener('click', function() {
      ...
      for (var l = 0; l < allItems.length; l++) {
        ...
        allItems[l].querySelector('.accordion-content').style.maxHeight = null;
      }
  ...
  var closePlanModal = document.getElementById('close-plan-modal');
  var modalTitle     = document.getElementById('modal-plan-title');
  var modalDetails   = document.getElementById('modal-plan-details');
  var modalWaBtn     = document.getElementById('modal-wa-btn');
  var marqueeTrack   = document.getElementById('marquee-track');

  var openPlanBtns = document.querySelectorAll('.open-plan-modal');
  for (var m = 0; m < openPlanBtns.length; m++) {
    openPlanBtns[m].addEventListener('click', function() {
      var key   = this.getAttribute('data-plan');
      var plan  = planData[key];
      var fotos = fotosPorPlan[key] || [];
      ...
      var incluyeHtml = '';
      for(var n=0; n<plan.incluye.length; n++) { incluyeHtml += '<p>✓ ' + plan.incluye[n] + '</p>'; }
      
      var terminosHtml = '';
      for(var o=0; o<plan.terminos.length; o++) { terminosHtml += '<p>' + plan.terminos[o] + '</p>'; }

      modalDetails.innerHTML = 
        '<h4>Descripción</h4><p>' + plan.detalles + '</p>' +
        '<h4>Incluye</h4>' + incluyeHtml +
        '<h4>Términos</h4><div class="terms-text">' + terminosHtml + '</div>';
        
      modalWaBtn.href = "https://wa.me/584123590065?text=" + encodeURIComponent(plan.msg);
      
      // Llenar el marquee uniendo el arreglo dos veces para el loop
      var fotosDobles = fotos.concat(fotos);
      var fotosHtml = '';
      for(var p=0; p<fotosDobles.length; p++) {
        fotosHtml += '<img src="' + fotosDobles[p] + '" alt="" loading="lazy"/>';
      }
      marqueeTrack.innerHTML = fotosHtml;
  ...
  planModal.addEventListener('click', function(e) { if (e.target === planModal) cerrarPlanModal(); });
  document.addEventListener('keydown', function(e) { if (e.key === 'Escape' && planModal.classList.contains('active')) cerrarPlanModal(); });
