document.addEventListener("DOMContentLoaded", function() {
  ...
  // 1. SCROLL SUAVE (Menú de navegación)
  const enlaces = document.querySelectorAll('nav ul li a[href^="#"]');
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll('.galeria-trigger').forEach(item => {
    item.addEventListener('click', function() {
      ...
      const srcIndividual = this.getAttribute('data-src') || this.querySelector('img').src;
      imagenesActuales = galeriasPorCategoria[cat] || [srcIndividual];
      abrirLightbox(0);
    });
  });
  ...
  document.querySelectorAll('.open-plan-modal').forEach(btn => {
    btn.addEventListener('click', function() {
      const key   = this.getAttribute('data-plan');
      ...
      let incluyeHtml = '';
      plan.incluye.forEach(item => { incluyeHtml += `<p>✓ ${item}</p>`; });
      
      let terminosHtml = '';
      plan.terminos.forEach(item => { terminosHtml += `<p>${item}</p>`; });
